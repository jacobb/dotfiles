return {
  {
    ...
    config = function ()
      ...
      local au = {}
      function au.declare_group(name, opts, aucmds)
      ...
      function M.get_sid(file)
        ...
        local script_entry = vim.api.nvim_exec("filter #autoload/fugitive.vim$# scriptnames", true)
        print("Script entry:", vim.inspect(script_entry))
        vim.notify("se" .. script_entry)
        vim.notify("match? " .. script_entry:match("^ ?(%d+)"))
        M.sid_cache[file] = tonumber(script_entry:match("^ ?(%d+)")) --[[@as integer ]]
        vim.notify("sid" .. M.sid_cache[file])
        ...
        },
        {
          ...
          callback = function (ctx)
            ...
            km.set("n", "DD", function ()
              ...
              if info then
                if #info.paths > 0 then
                  vim.cmd(fmt("DiffviewOpen --selected-file=%s",
                    vim.fn.fnameescape(info.paths[1])))
                elseif info.commit ~= "" then
